         ...
         <div class="col-md-4 mx-auto">
            <div class="myform form login-form">
              ...
              <div class="text-center ">
                <?php
                $id=$_GET['id'];
                $query = "SELECT * from regester where id = '$id'"; 
                $result = mysqli_query($db, $query);
                if (mysqli_num_rows($result) > 0) {
                $row = mysqli_fetch_assoc($result);

                echo "
                  <span class= 'header2'>You can content with ".$row['username']." by his information  </span>
                  <div calss = 'form-group'>
                  <label>phone </label> 
                  <input type='text' disabled class = ' form-control my-input' value=".$row['phone'].">
                  </div>
                  <div calss = 'form-group'>
                  <label>Email  </label> 
                  <input type='text' disabled class = ' form-control my-input' value=".$row['email'].">
                  </div>
                ...
                <form action ="login.php">
                     <button type="submit"name ="home" class=" btn btn-block send-button tx-tfm m-t-12">Home Page </button>
                  </form>
              </div>
